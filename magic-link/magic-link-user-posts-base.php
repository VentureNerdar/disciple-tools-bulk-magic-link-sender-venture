...
?>
    ...
    public function post_form($post, $fields, $post_settings, $connections_enabled) {
        ...
        if (!empty($fields) && !empty($this->post_field_settings)) {
            ...
            // NERDAR CHANGES
            $sort_orders = Disciple_Tools_Bulk_Magic_Link_Sender_API::magic_link_fields_sort_order();

            $type_name = $post['post_type'] === 'groups' ? 'smart_links_user_groups_updates_magic_key' : 'smart_links_user_contacts_updates_magic_key';
            $search_key = 'type_key';

            $type_sort_order = array_values(array_filter($sort_orders, function ($element) use ($search_key, $type_name) {
                return isset($element->$search_key) && $element->$search_key == $type_name;
            }));

            $order = $type_sort_order[0]->sort_order;
            $increment = 0;
            $updated_fields = [];
            foreach ($order as $o) {
                $increment++;
                ...
                foreach ($fields as $field) {
                    if ($field['id'] === $v) {
                        $format_field = $field;
                        $format_field['sort_order'] = (int) $o->key;
                    }
                }

                array_push($updated_fields, $format_field);
            }
